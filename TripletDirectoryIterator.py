...
class TripletDirectoryIteratorForC2S(image.DirectoryIterator):
    def __init__(self, directory, image_data_generator,
        ...
        self.num_attrs = num_attrs
        self.num_bbox = 4
        self.base_path = "./data_c2s/"
        self.pairs = dict()

        with open(os.path.join(self.base_path, 'Eval/list_eval_partition.txt'), 'r') as eval_partition_file:
    ...
    def next(self):
        ...
        for i in range(self.batch_size):
            ...
            # print("Anchor image")
            idx_1 = rng.randint(0, self.samples)
            while "comsumer" not in self.filenames[idx_1]:
                idx_1 = rng.randint(0, self.samples)
            # print(self.filenames[idx_1])
            anchor_item_idx = str(self.filenames[idx_1]).split("/")[-2]
            pairs[0][i, :, :, :] = self.get_image(idx_1)
            batch_y[0][i, self.classes[idx_1]] = 1
            ...
            # pick positive and negative samples to anchor image.
            # print("Positive image")
            idx_2 = self.filenames.index(self.pairs[self.filenames[idx_1]])
            # print(self.filenames[idx_2])
            pairs[1][i, :, :, :] = self.get_image(idx_2)
            batch_y[1][i, self.classes[idx_2]] = 1
